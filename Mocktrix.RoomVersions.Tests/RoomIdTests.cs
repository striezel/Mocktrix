...
{
    /// <summary>
    /// Contains tests for RoomId class.
    /// </summary>
    public class RoomIdTests
    {
        ...
        {
            var id = RoomId.Generate(new Uri("https://matrix.example.org/"));
            Assert.NotNull(id);
            Assert.NotEmpty(id);
        ...
        public void Generate_IdContainsServer()
        {
            // localhost
            {
                var id = RoomId.Generate(new Uri("http://localhost:8080/"));
                Assert.NotNull(id);
                Assert.NotEmpty(id);
                Assert.StartsWith("!", id);
            ...
            // foo.example.com
            {
                var id = RoomId.Generate(new Uri("https://foo.example.com"));
                Assert.NotNull(id);
                Assert.NotEmpty(id);
                Assert.StartsWith("!", id);
            ...
            // some.domain.tld
            {
                var id = RoomId.Generate(new Uri("https://some.domain.tld"));
                Assert.NotNull(id);
                Assert.NotEmpty(id);
                Assert.StartsWith("!", id);
        ...
        public void Generate_Twice()
        {
            ...
            var id1 = RoomId.Generate(uri);
            var id2 = RoomId.Generate(uri);
