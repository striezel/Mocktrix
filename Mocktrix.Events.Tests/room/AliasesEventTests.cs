...
{
    /// <summary>
    /// Contains tests for AliasesEvent.
    /// </summary>
    public class AliasesEventTests
    {
        [Fact]
        public void Construction()
        {
            var ev = new AliasesEvent();
            Assert.NotNull(ev.Content);
            Assert.IsType<AliasesEventContent>(ev.Content);
            ...
            Assert.Equal("m.room.aliases", ev.Type);
        ...
        public void DeserializeSpecExample()
        {
            var json = """
                       ...
                       """;
            var ev = JsonSerializer.Deserialize<AliasesEvent>(json);
            ...
            Assert.Equal("example.org", ev.StateKey);
            Assert.Equal("m.room.aliases", ev.Type);
        ...
        public void SerializeSpecExample()
        {
            var ev = new AliasesEvent
            {
                Content = new AliasesEventContent
                {
            ...
            var expected_json = "{\"content\":{\"aliases\":[\"#somewhere:example.org\",\"#another:example.org\"]},\"event_id\":\"$143273582443PhrSn:example.org\",\"origin_server_ts\":1432735824653,\"room_id\":\"!jEsUZKDJdhlrceRyVU:example.org\",\"sender\":\"@example:example.org\",\"state_key\":\"example.org\",\"type\":\"m.room.aliases\",\"unsigned\":{\"age\":1234}}";
        ...
        {
            Assert.True(new AliasesEvent().IsStateEvent());
        }
    }
}
