...
{
    /// <summary>
    /// Contains tests for RoomAvatarEvent.
    /// </summary>
    public class RoomAvatarEventTests
    {
        [Fact]
        public void Construction()
        {
            var ev = new RoomAvatarEvent();
            Assert.NotNull(ev.Content);
            Assert.IsType<RoomAvatarEventContent>(ev.Content);
            ...
            Assert.Equal("m.room.avatar", ev.Type);
        ...
        public void DeserializeSpecExample()
        {
            var json = """ 
                       ...
                       """;
            var ev = JsonSerializer.Deserialize<RoomAvatarEvent>(json);
        ...
        public void SerializeSpecExample()
        {
            var ev = new RoomAvatarEvent
            {
                Content = new RoomAvatarEventContent
                {
                    Info = new ImageInfo()
                    {
                        Height = 398,
            ...
            var expected_json = "{\"content\":{\"info\":{\"h\":398,\"mimetype\":\"image/jpeg\",\"size\":31037,\"w\":394},\"url\":\"mxc://example.org/JWEIFJgwEIhweiWJE\"},\"event_id\":\"$143273582443PhrSn:example.org\",\"origin_server_ts\":1432735824653,\"room_id\":\"!jEsUZKDJdhlrceRyVU:example.org\",\"sender\":\"@example:example.org\",\"state_key\":\"\",\"type\":\"m.room.avatar\",\"unsigned\":{\"age\":1234}}";
        ...
        {
            Assert.True(new RoomAvatarEvent().IsStateEvent());
        }
    }
}
