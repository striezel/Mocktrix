...
{
    /// <summary>
    /// Contains tests for EmoteMessageEvent.
    /// </summary>
    public class EmoteMessageEventTests
    {
        [Fact]
        public void Construction()
        {
            var ev = new EmoteMessageEvent();
            Assert.NotNull(ev.Content);
            Assert.IsType<EmoteMessageEventContent>(ev.Content);
            Assert.Null(ev.Content.Body);
            Assert.Null(ev.Content.Format);
            Assert.Null(ev.Content.FormattedBody);
            Assert.Equal("m.emote", ev.Content.MessageType);
        ...
        public void DeserializeSpecExample()
        {
            var json = """ 
                       ...
                       """;
            var ev = JsonSerializer.Deserialize<EmoteMessageEvent>(json);

            Assert.NotNull(ev);
            Assert.NotNull(ev.Content);
            Assert.Equal("thinks this is an example emote", ev.Content.Body);
            Assert.Equal("org.matrix.custom.html", ev.Content.Format);
            Assert.Equal("thinks <b>this</b> is an example emote", ev.Content.FormattedBody);
            Assert.Equal("m.emote", ev.Content.MessageType);
        ...
        public void SerializeSpecExample()
        {
            var ev = new EmoteMessageEvent
            {
                Content = new EmoteMessageEventContent
                {
                    Body = "thinks this is an example emote",
                    Format = "org.matrix.custom.html",
        ...
        {
            Assert.False(new EmoteMessageEvent().IsStateEvent());
        }
    }
}
