...
{
    /// <summary>
    /// Contains tests for FileMessageEvent.
    /// </summary>
    public class FileMessageEventTests
    {
        [Fact]
        public void Construction()
        {
            var ev = new FileMessageEvent();
            Assert.NotNull(ev.Content);
            Assert.IsType<FileMessageEventContent>(ev.Content);
            Assert.Null(ev.Content.Body);
            Assert.Null(ev.Content.Info);
            Assert.Equal("m.file", ev.Content.MessageType);
        ...
        public void DeserializeSpecExample()
        {
            var json = """ 
                       ...
                       """;
            var ev = JsonSerializer.Deserialize<FileMessageEvent>(json);

            Assert.NotNull(ev);
            Assert.NotNull(ev.Content);
            Assert.Equal("something-important.doc", ev.Content.Body);
            Assert.Equal("something-important.doc", ev.Content.FileName);
            Assert.NotNull(ev.Content.Info);
            Assert.Equal("application/msword", ev.Content.Info.MimeType);
            Assert.Equal(46144, ev.Content.Info.Size);
            Assert.Null(ev.Content.Info.ThumbnailInfo);
            Assert.Null(ev.Content.Info.ThumbnailUrl);
            Assert.Equal("m.file", ev.Content.MessageType);
            Assert.Equal("mxc://example.org/FHyPlCeYUSFFxlgbQYZmoEoe", ev.Content.Url);
        ...
        public void SerializeSpecExample()
        {
            var ev = new FileMessageEvent()
            {
                Content = new FileMessageEventContent()
                {
                    Body = "something-important.doc",
                    FileName = "something-important.doc",
                    Info = new FileInfo()
            ...
            var expected_json = "{\"content\":{\"body\":\"something-important.doc\",\"filename\":\"something-important.doc\",\"info\":{\"mimetype\":\"application/msword\",\"size\":46144},\"msgtype\":\"m.file\",\"url\":\"mxc://example.org/FHyPlCeYUSFFxlgbQYZmoEoe\"},\"event_id\":\"$143273582443PhrSn:example.org\",\"origin_server_ts\":1432735824653,\"room_id\":\"!jEsUZKDJdhlrceRyVU:example.org\",\"sender\":\"@example:example.org\",\"type\":\"m.room.message\",\"unsigned\":{\"age\":1234}}";
        ...
        {
            Assert.False(new FileMessageEvent().IsStateEvent());
        }
    }
}
