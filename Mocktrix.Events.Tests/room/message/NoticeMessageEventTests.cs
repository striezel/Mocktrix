...
{
    /// <summary>
    /// Contains tests for NoticeMessageEvent.
    /// </summary>
    public class NoticeMessageEventTests
    {
        [Fact]
        public void Construction()
        {
            var ev = new NoticeMessageEvent();
            Assert.NotNull(ev.Content);
            Assert.IsType<NoticeMessageEventContent>(ev.Content);
            Assert.Null(ev.Content.Body);
            Assert.Null(ev.Content.Format);
            Assert.Null(ev.Content.FormattedBody);
            Assert.Equal("m.notice", ev.Content.MessageType);
        ...
        public void DeserializeSpecExample()
        {
            var json = """ 
                       ...
                       """;
            var ev = JsonSerializer.Deserialize<NoticeMessageEvent>(json);

            Assert.NotNull(ev);
            Assert.NotNull(ev.Content);
            Assert.Equal("This is an example notice", ev.Content.Body);
            Assert.Equal("org.matrix.custom.html", ev.Content.Format);
            Assert.Equal("This is an <strong>example</strong> notice", ev.Content.FormattedBody);
            Assert.Equal("m.notice", ev.Content.MessageType);
        ...
        public void SerializeSpecExample()
        {
            var ev = new NoticeMessageEvent
            {
                Content = new NoticeMessageEventContent
                {
                    Body = "This is an example notice",
                    Format = "org.matrix.custom.html",
        ...
        {
            Assert.False(new NoticeMessageEvent().IsStateEvent());
        }
    }
}
