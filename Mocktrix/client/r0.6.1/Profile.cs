...
namespace Mocktrix.client.r0_6_1
{
    ...
    public static class Profile
    {
        ...
        {
            // Adds https://spec.matrix.org/historical/client_server/r0.6.1.html#get-matrix-client-r0-profile-userid-displayname,
            // i.e. the endpoint to query a user's display name.
            app.MapGet("/_matrix/client/r0/profile/{userId}/displayname", GetUserDisplayName);

            // Add https://spec.matrix.org/historical/client_server/r0.6.1.html#put-matrix-client-r0-profile-userid-displayname,
            // i.e. the possibility to change the own display name.
            app.MapPut("/_matrix/client/r0/profile/{userId}/displayname", async (string userId, HttpContext context) =>
            {
                return await ChangeUserDisplayName(userId, context, app);
            });

            // Add https://spec.matrix.org/historical/client_server/r0.6.1.html#get-matrix-client-r0-profile-userid-avatar-url,
            // i.e. the possibility to query a user's avatar URL.
            app.MapGet("/_matrix/client/r0/profile/{userId}/avatar_url", GetUserAvatar);

            // Add https://spec.matrix.org/historical/client_server/r0.6.1.html#put-matrix-client-r0-profile-userid-avatar-url,
            // i.e. the possibility to change the own avatar URL.
            app.MapPut("/_matrix/client/r0/profile/{userId}/avatar_url", async (string userId, HttpContext context) =>
            {
                return await ChangeUserAvatar(userId, context, app);
            });

            // Implement https://spec.matrix.org/historical/client_server/r0.6.1.html#get-matrix-client-r0-profile-userid,
            // i.e. the endpoint to get a user's profile information.
            app.MapGet("/_matrix/client/r0/profile/{userId}", GetUserProfileData);
        }
    }
}
