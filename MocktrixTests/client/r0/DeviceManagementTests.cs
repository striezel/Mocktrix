...
namespace MocktrixTests
{
    public class DeviceManagementTests
    {
        ...
        public async Task TestDeviceWithId_WithAuthorization()
        {
            ...
            {
                body.device_id,
                display_name = "My device mgmt. dev #1",
                last_seen_ip = nullString,
                last_seen_ts = nullInt64
            };
            var content = Utilities.GetContent(response, expected_response);
            Assert.Equal(expected_response.device_id, content.device_id);
            Assert.Equal(expected_response.display_name, content.display_name);
            Assert.Null(content.last_seen_ip);
            Assert.Null(content.last_seen_ts);
        }
    }
}
